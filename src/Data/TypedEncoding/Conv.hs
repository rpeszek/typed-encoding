...
-- @
--
-- @"r-UNICODE.D76"@ and @"r-UTF8"@ is considered redundant for @T.Text@ and can be added and dropped as needed.
-- 
...
-- @  
--
-- So what is the set of common values allowing us to use any of these 2 options?
-- "r-UNICODE.D76" is not important here, what is the intersection of /UTF8/ and /CHAR8/ code point space?
--
-- There are many character set encodings that utilize one byte (/CHAR8/) and /UTF8/ is different from all of them
-- but it backward compatible with the /ASCII/ range of chars @ < 127@.  So the intersection should be /ASCII/, let us check that:
--
-- @
...
-- 8-bit encoding like /ISO/IEC 8859/ family of encodings.  This effectively prevents these bytes
...
--
-- It is quite possible to try to enforce validity of the stack for @str2@, this is currently not implemented.
