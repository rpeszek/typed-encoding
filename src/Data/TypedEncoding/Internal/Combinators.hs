...
foldEnc :: forall (xs2 :: [k]) (xs1 :: [k]) f c s1 s2 . (Foldable f, Functor f) => c -> (s1 -> s2 -> s2) -> s2 -> f (Enc xs1 c s1) -> Enc xs2 c s2
...
foldEncStr :: forall (xs2 :: [k]) (xs1 :: [k]) f c s1 s2 . (Foldable f, Functor f, IsString s2) => c -> (s1 -> s2 -> s2) -> f (Enc xs1 c s1) -> Enc xs2 c s2
foldEncStr c f = foldEnc c f ""

-- TODO pass SomeAnn to fold
-- | Similar to 'foldEnc', works with untyped 'SomeEnc'
foldSomeEnc :: forall (xs2 :: [k]) f c s1 s2 . (Foldable f, Functor f) => c -> (SomeAnn -> s1 -> s2 -> s2) -> s2 -> f (SomeEnc c s1) -> Enc xs2 c s2
...
-- unreduce :: Enc '["r-IPv4"] () T.Text -> f (IpV4F (Enc '["r-Word8-decimal"] () T.Text))
